...
2. **UPS Module**:
   ...
   * DC barrel jack: Left wall port ($12.0\text{ mm}$ wide along Y, $16.0\text{ mm}$ tall) and matching cradle cutout modeled with a $5.10\text{ mm}$ solid half-wall threshold from $Z = 0 \to 5.10\text{ mm}$ (`ups_total_h`), perfectly flush with the UPS cradle perimeter walls and the top of the PCB. Completely conceals the $1.6\text{ mm}$ green PCB edge, blocks the view underneath the PCB, and provides a continuous support beam preventing PCB flex when inserting power plugs.
